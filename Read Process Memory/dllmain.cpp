...
bool DrawMessage(unsigned int x, unsigned int y, int alpha, unsigned char r, unsigned char g, unsigned char b, LPCSTR Message)
{	// Create a colour for the text
    ...
    DrawTextA(NULL, Message, -1, &rct, DT_CENTER);
    return true;
}
...
)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        if (GetAsyncKeyState(VK_INSERT) & 0x80000)
        {
            imguiDraw = 1 - imguiDraw;
        }

        while (imguiDraw)
        {
            HRESULT RestoreAllSurfaces();
            DrawMessage(120, 300, 100, 255, 0, 0, "Lool");
        }
